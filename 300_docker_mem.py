...
cmd1 = "/usr/bin/sh /usr/local/fountain/docker/docker-tools_centos7.2/docker-monitor -m"
cmd2 = "/usr/bin/sh /home/work/open-falcon/agent/plugin/docker/docker-monitor -m"
...
try:
    value=float(re[2].split()[3].split('%')[0])
    create_record(value)
except Exception,err:
    status = 0
    logging.error("Run command failed:%s" %str(err))
data.append({"endpoint": endpoint, "metric": "monitor.scripts.health", "timestamp": ts, "step": 300, "value": status, "counterType": "GAUGE", "tags": "scripts=300_docker_mem.py"})
