...
for pid in pid_list:
    try:
        proc_fd_maxnum=commands.getoutput("cat /proc/%s/limits | awk '/^Max open files/{print $4}'" %pid)
...
counter_list.append({"endpoint": endpoint, "metric": "monitor.scripts.health", "timestamp": ts, "step": step, "value": status, "counterType": "GAUGE", "tags": "scripts=3600_maxfdpercent_process.py"})
